...
// example define a parameter in which we can pass something to it that it should
// print to the console, along with a default message:



function sayHello(somethingToSay) {
  // Note that console.log() is a function just like sayHello(), but it is not
  // defined by us, it's provided by Scriptographer, so we can print something
  // to the console. And console.log() can receive parameters just like sayHello(),
  // I is more flexible and can receive a variable amount, which it just prints
